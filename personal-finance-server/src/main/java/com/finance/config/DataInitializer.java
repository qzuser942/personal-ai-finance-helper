...
public class DataInitializer implements CommandLineRunner {
        ...
        private void initAiConfig() {
                // === DeepSeek大模型配置（逐个Key检查，支持补建缺失项） ===
                insertAiConfigIfAbsent("deepseek_api_key",
                                "${DEEPSEEK_API_KEY:sk-your-deepseek-api-key}",
                                "STRING", "DeepSeek官方API密钥（通过环境变量DEEPSEEK_API_KEY配置）");
                insertAiConfigIfAbsent("model_base_url",
                                "https://api.deepseek.com/v1",
                                "STRING", "DeepSeek官方API地址（OpenAI兼容接口）");
                insertAiConfigIfAbsent("model_name",
                                "deepseek-chat",
                                "STRING", "大模型名称：deepseek-chat / deepseek-reasoner");
                insertAiConfigIfAbsent("model_temperature",
                                "0.7",
                                "NUMBER", "模型temperature参数（0-2），控制输出随机性");
                insertAiConfigIfAbsent("model_max_tokens",
                                "4096",
                                "NUMBER", "模型max_tokens参数，最大输出长度");
                insertAiConfigIfAbsent("model_top_p",
                                "0.9",
                                "NUMBER", "模型top_p核采样参数");

                // === 阿里云百炼嵌入配置 ===
                insertAiConfigIfAbsent("dashscope_api_key",
                                "${DASHSCOPE_API_KEY:sk-your-dashscope-api-key}",
                                "STRING", "阿里云百炼DashScope API密钥（通过环境变量DASHSCOPE_API_KEY配置）");
                insertAiConfigIfAbsent("embedding_model",
                                "text-embedding-v2",
                                "STRING", "阿里云百炼文本嵌入模型名称");
                insertAiConfigIfAbsent("embedding_dimension",
                                "1536",
                                "NUMBER", "向量维度（text-embedding-v2 = 1536）");

                // === Qdrant向量数据库配置 ===
                insertAiConfigIfAbsent("qdrant_host",
                                "localhost",
                                "STRING", "Qdrant向量数据库服务地址");
                insertAiConfigIfAbsent("qdrant_port",
                                "6334",
                                "NUMBER", "Qdrant gRPC通信端口");
                insertAiConfigIfAbsent("qdrant_collection",
                                "user_consumption_vectors",
                                "STRING", "Qdrant向量集合名称");

                // === Prompt模板 ===
                insertAiConfigIfAbsent("prompt_template_analysis",
                ...
                insertAiConfigIfAbsent("prompt_template_classify",
                ...
                insertAiConfigIfAbsent("prompt_template_feature_extraction",
        ...
        private void insertAiConfigIfAbsent(String key, String value, String type, String desc) {
                Long count = aiConfigMapper.selectCount(
                                new LambdaQueryWrapper<AiConfig>().eq(AiConfig::getConfigKey, key));
                if (count > 0) {
                        log.debug("AI配置Key '{}' 已存在，跳过", key);
                        return;
                }
                insertAiConfig(key, value, type, desc);
        }
