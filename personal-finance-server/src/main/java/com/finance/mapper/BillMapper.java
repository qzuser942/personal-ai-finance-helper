...
public interface BillMapper extends BaseMapper<Bill> {
        ...
        @Select("SELECT DATE_FORMAT(consume_time, '%Y-%m') AS yearMonth, " +
                        ...
                        "COALESCE(SUM(amount), 0) AS totalAmount " +
                        "FROM bill WHERE is_deleted = 0 " +
                        "AND consume_time >= DATE_SUB(NOW(), INTERVAL 12 MONTH) " +
                        "GROUP BY DATE_FORMAT(consume_time, '%Y-%m') ORDER BY yearMonth")
        List<Map<String, Object>> recent12MonthTrend();

        /**
         * 异常消费用户：单月消费超月均3倍
         */
        @Select("SELECT u.username, SUM(b.amount) AS monthlyAmount, " +
                        "(SELECT COALESCE(SUM(m.amount),0) FROM bill m WHERE m.user_id=b.user_id AND m.is_deleted=0 AND m.consume_time>=DATE_SUB(NOW(),INTERVAL 6 MONTH))/6 AS avgMonthly "
                        +
                        "FROM bill b LEFT JOIN sys_user u ON b.user_id=u.id " +
                        "WHERE b.is_deleted=0 AND DATE_FORMAT(b.consume_time,'%Y-%m')=#{yearMonth} " +
                        "GROUP BY b.user_id,u.username HAVING monthlyAmount>avgMonthly*3 AND avgMonthly>0 " +
                        "ORDER BY monthlyAmount DESC LIMIT 10")
        List<Map<String, Object>> anomalyUsers(@Param("yearMonth") String yearMonth);
